...
| `scraper/config/scraper.json` | Board-specific config - `apiBase`, `apiPath`, `department`, `jobDetailsPrefix` |
...
| `tests/unit/test_api.py` | Unit tests - query/upsert/delete, HTTP error handling, verification scoping |
...
| `tests/consistency/test_readme.py` | README conformity checks (Romanian content, badges, license) |
| `tests/consistency/test_code_of_conduct.py` | Code of Conduct full Covenant 2.1 checks |
...
- Stale-deletion is scoped to the applytojob board prefix only, so jobs
  published by other peviitor scrapers under the same CIF are never removed.
- Full workflow: validate company (ANAF) → scrape ELECTROGRUP board → merge
