...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/electrogrup-sa-python-scraper --json status -q .status
```
...
(`gh issue list --repo peviitor-scrapers/electrogrup-sa-python-scraper --state open`),
