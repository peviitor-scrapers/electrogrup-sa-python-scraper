...
This scraper extracts job listings for **ELECTROGRUP SA** (CIF `9256208`)
from the applytojob board, filtered by the `ELECTROGRUP` department,
and publishes them to peviitor.ro via the v1 API (`https://api.peviitor.ro/v1`).

Target: `https://electrogrup.applytojob.com/apply/jobs/?department=ELECTROGRUP`
...
1. **Start with brand** — `ELECTROGRUP`
2. **Search in ANAF/CUIScan** — find company by CIF `9256208`
...
7. **Scrape new jobs** — parse the ELECTROGRUP applytojob board (department filter)
...
> published by other scrapers under the same CIF are preserved.
...
4. **Scrape jobs** — parse the ELECTROGRUP applytojob board
...
company.py / anaf.py (validate company)
    ...
    │
    ▼ (if active)
scrape applytojob board (?department=ELECTROGRUP)
    │
    ▼
...
| `scraper/config/scraper.json` | Board config — `apiBase`, `apiPath`, `department`, `jobDetailsPrefix` |
...
- **Applytojob board**: `https://electrogrup.applytojob.com/apply/jobs/?department=ELECTROGRUP` — listing HTML
...
# so jobs from other scrapers under the same CIF are never touched)
python3 -m scraper.api 9256208

# Validate job URLs from peviitor by CIF (head/content/browser)
python3 -m scraper.validate_jobs 9256208 --mode head
